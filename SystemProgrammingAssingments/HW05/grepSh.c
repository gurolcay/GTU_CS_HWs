/*
 * HW05_121044029_Mehmet_Gurol_Cay
 *
 * File: main.c
...
int main(int argc, char const *argv[]) {
    ...
    if (argc != 3) {
        fprintf(stdout, "Usage: %s \"string\" <DirPath>\n",argv[0] );
        return 1;
    }
    ...
    // if((messageQueueKey = ftok("grepSh.c",1)) == (key_t) -1){
    //     perror("messageQueueKey couldn't creat");
    //     exit(EXIT_FAILURE);
    // }

    messageQueueKey = (key_t) getpid();

    if((msqid = msgget(messageQueueKey, PERMS | IPC_CREAT))){
	...
	if(childpid == 0){
        ...
        for (i = 0; i < dirPathCounter; i++){
            ...
            //read message queue
            if((size = msgrcv(msqid,&processData,sizeof(processData),0,0)) == ERROR){
...
void printToTerminal(struct OutputData outputData){
    ...
    fclose(pFileOutput);
    (void) fprintf(stdout,"Total number of strings found:         %d\n",counter);
    (void) fprintf(stdout,"Number of directories searched:        %d\n",outputData.totalSearchedDirNum );
    (void) fprintf(stdout,"Number of files searched:              %d\n",outputData.totalSearchedFileNum );
    (void) fprintf(stdout,"Number of lines searched:              %d\n",outputData.totalSearchedLineNum);
    (void) fprintf(stdout,"Number of cascade threads created:     %d\n",outputData.totalCascadeThreadNum);
    (void) fprintf(stdout,"Number of search threads created:      %d\n",outputData.totalSearcherThreadNum);
    (void) fprintf(stdout,"Max %d of threads running concurrently.  \n",outputData.totalThreadNum);
    (void) fprintf(stdout,"Total run time, %d in milliseconds.      \n",outputData.totalRunTime );
...
void startProcess(char dirPath[BUF_SIZE], int wPipeFD, const char *searchedWord){
    ...
    *sharedtotal = 0;
    ...
    processData.liveThreadNum = *sharedtotal;
    ...
    #endif
    //write message queue
    if(msgsnd(msqid,&processData,sizeof(processData),0) == ERROR){
...
void *startThread(void *datas){
    ...
    //sleep(1);
    if ((sharedtotal = (int *)shmat(threadData->sharedID, NULL, 0)) == (void *)-1) {
      perror("Failed to attach shared memory segment");
    }
...
/*
* http://usp.cs.utsa.edu/usp/programs/chapter15/detachandremove.c
*/
