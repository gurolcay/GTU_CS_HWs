 ...
 * I added to encode method to Huffman tree.
 *
 * @author Koffman and Wolfgang edited by Mehmet Gurol Cay
 *         <p>
 *         Created by Mehmet Gurol Cay on 19.04.2017.
 */
public class MyHuffmanTree implements Serializable {
    ...
    public String encode(String string) {
        ...
        for (int i = 0; i < string.length(); i++) {
            encodedString += encode(string.charAt(i),"",huffTree);
        }
     ...
     * @return
     */
    private String encode(Character chr, String temp ,BinaryTree<HuffData> tree){
        HuffData theData = tree.getData();
        String coded = "";
        if (theData.symbol != null){
            if (theData.symbol.equals(chr)){
                return temp;
            }
        }else {
            coded += encode(chr, temp + "0", tree.getLeftSubtree());
            coded += encode(chr, temp + "1" ,tree.getRightSubtree());
        }
        return coded;
    }
    ...
    public void readFromFile() {
        try {
            BufferedReader bf = new BufferedReader(new FileReader("freq.txt"));
            String line;
            int i = 0;
            HuffData[] symbolsArr = new HuffData[27];
            HuffData symbol;
            ...
            while ((line = bf.readLine()) != null){
                ...
                if (tokens[0].equals("_")){
                    symbol = new HuffData(186,' ');
                }else {
                    symbol = new HuffData(Double.parseDouble(tokens[1]),tokens[0].charAt(0));
                }
                symbolsArr[i] = symbol;
                ++i;
            }

            buildTree(symbolsArr);
